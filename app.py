...
#----------------- 12 aug 2025 -------  the basic thing of size limit
from flask import Flask, render_template, request, session, redirect, jsonify
import os
from db import get_db_connection
from extraction import submit
...
def check_resume():
    ...
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id FROM resumes WHERE candidate_email = %s LIMIT 1", (candidate_email,))
    existing_resume = cursor.fetchone()
    conn.close()

    return jsonify({"exists": bool(existing_resume)})
...
def upload_resume():
    ...
    if file_size > 5 * 1024 * 1024:
        return jsonify({"error": "File too large! Max 5 MB allowed."}), 400

    # Save or replace logic
    submit()  # Your function to save file + DB
